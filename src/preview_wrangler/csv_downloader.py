...
class CSVDownloader:
    ...
    def download_csv_files(self, manifest: InventoryManifest) -> list[Path]:
        ...
        logger.info(
            f"Downloading {len(manifest.files)} CSV files (up to {self.MAX_WORKERS} concurrent)"
        )
    ...
    def _download_single_csv(self, file_info: InventoryFile) -> Optional[Path]:
        ...
        if self.cache_manager.is_cached(file_info.key, file_info.md5_checksum):
            ...
            if cached_path is None:
                logger.debug(f"Cached path not found for {file_info.key}")
            else:
        ...
        try:
            ...
            # Add to cache
            self.cache_manager.add_to_cache(file_info.key, gzip_path, file_info.md5_checksum)
